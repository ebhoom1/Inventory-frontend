...
const UserList = () => {
  ...
  const rows = useMemo(() => {
    ...
    if (filterMode === "technicians") {
      return allUsers.filter((u) => u.userType === "Technician");
    }
      ...
      html: `
        <div style="text-align: center; padding: 20px;">
          ...
          <p style="font-size: 18px; color: #374151; margin-bottom: 15px; font-weight: 600;">
            Are you sure you want to delete <strong style="color: #dc2626;">"${
              userName || "this user"
            }"</strong>?
          </p>
  ...
  return (
    <div className="mt-10">
      ...
      {/* Header + Filter Toggle (Admins see Users+Technicians, Super Admins see all 3) */}
      <div className="mb-4 flex items-center justify-between">
        ...
        {(userInfo?.userType === "Admin" || isSuperAdmin) && (
          <div className="inline-flex rounded-lg overflow-hidden border border-orange-300">
            {/* Users button (always visible for Admins + Super Admins) */}
            <button
            ...
            {/* Admins button (ONLY Super Admin can see this) */}
            {isSuperAdmin && (
              <button
            ...
            {/* Technicians button (always visible for Admins + Super Admins) */}
            <button
              onClick={() => setFilterMode("technicians")}
              className={`px-4 py-2 text-sm font-medium transition ${
                filterMode === "technicians"
                  ? "bg-[#DC6D18] text-white"
                  : "bg-white text-[#DC6D18] hover:bg-orange-50"
