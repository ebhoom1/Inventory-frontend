...
import { logInventoryUsage, resetInventoryState, fetchInventory } from '../../redux/features/inventory/inventorySlice';
...
function UseInventory() {
  ...
  useEffect(() => {
    const fetchUserSkus = async () => {
      ...
      try {
        ...
        const res = await fetch(`${API_URL}/api/inventory/user/${encodeURIComponent(userInfo.userId)}`, {
        ...
        const uniqueSkus = Array.from(
          new Set((Array.isArray(data) ? data : []).map((it) => it.skuName).filter(Boolean))
  ...
  useEffect(() => {
    if (usersError) {
      Swal.fire({ icon: 'warning', title: 'Could not load users', text: usersError });
    }
  }, [usersError]);
      ...
      <form className="space-y-10" onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-10">
          ...
          <div className="relative flex items-center">
            ...
            >
              <option value="">
                {skuLoading ? 'Loading SKUs…' : (skuError ? 'Failed to load SKUs' : 'Select SKU')}
              </option>
              {!skuLoading && !skuError && skuOptions.map((name) => (
