...
// const EquipmentDetailsRow = ({ item, isAdmin,isUser,isSuperAdmin,onDownloadQR, onAddReport }) => {
...
//           >
//             {isAdmin ? "Add REport" : isSuperAdmin?"View Report":""}
//           </button>
//         </td>
//       </tr>
...
//                 <p className="text-sm text-gray-600"><strong>REF Due:</strong> {safeDate(item.refDue)}</p>
//               </div>
...
//     if (isAdmin) {
//       setReportListOpen(true); // admin: open viewer
//     } else {
//       setReportOpen(true); // non-admin: open create modal
//     }
//   };

//   const handleReportSaved = () => {
//     // optional: refresh or toast after save
//     // dispatch(fetchReportsByEquipment({ equipmentId: selectedItem?._id || selectedItem?.equipmentId }));
//   };
...
//           {loading && <div className="p-3 text-sm text-gray-600">Loading equipments…</div>}
...
const EquipmentDetailsRow = ({ item, isAdmin, isSuperAdmin, onDownloadQR, onAddReport }) => {
  const [isOpen, setIsOpen] = useState(false);
  const safeDate = (d) => (d ? new Date(d).toLocaleDateString() : "-");
  ...
  return (
    <>
      ...
      <tr className="hover:bg-orange-50/50">
        ...
        {/* Actions column only for Admin / Super Admin */}
        {showActions && (
          <td className="px-6 py-4 whitespace-nowrap text-sm text-center">
            <button
              onClick={() => onAddReport(item)}
              className="px-4 py-2 bg-[#DC6D18] text-white text-xs font-semibold rounded-lg shadow-md hover:bg-[#B85B14] transition-colors"
            >
              {isAdmin ? "Add Report" : "View Report"}
            </button>
          </td>
        )}
      </tr>
