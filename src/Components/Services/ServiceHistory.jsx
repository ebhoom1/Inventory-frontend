...
export default function ServiceHistory() {
  ...
  // dataset to render
  const data = isManager ? all : byUser;
  const items = data?.items || [];
  const total = data?.total || 0;
  const totalPages = data?.totalPages || 1;
  ...
  }, [dispatch, isManager, filter.month, filter.year, page, limit]);
  ...
  const totalCols = canModerate ? 10 : 9;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <h2 className="text-3xl font-bold text-gray-800">
          {isManager ? "All Service Requests" : "My Service Requests"}
        </h2>
      ...
      {/* Update Modal (Super Admin only) */}
      <Modal
        open={canModerate && editOpen}
        onClose={() => setEditOpen(false)}
        title={`Update Status — ${editing?.equipmentName || ""}`}
      >
        <form onSubmit={submitEdit} className="space-y-4">
